  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h(UBadge, { class: 'capitalize', variant: 'subtle', color }, () =>
        t(`customers.status.${row.original.status}`)
      )
    }
  },
  {
...
<template>
  <UDashboardPanel id="customers">
    <template #header>
      <UDashboardNavbar :title="t('common.customers')">
        <template #leading>
          ...
          <UDropdownMenu
            :items="
              table?.tableApi
                ...
                .map((column: any) => ({
                  label: t(`customers.columns.${column.id}`),
